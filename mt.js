...
(function () {
  ...
  function _initializeVariables() {
    ...
    loadingIcon = "<img title='Loading...' src='http://linker.in/mtos/mt-static/images/indicator.white.gif' alt='Loading' />";
  ...
  function _updateComments() {
    ...
    jsonUrl = [
        "http://linker.in/mtos/mt-comments.cgi?__mode=comment_listing&direction=",
        direction,
        "&entry_id=",
        entryID,
        "&limit=",
...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://linker.in/mtos/mt-comments.cgi?__mode=session_js&blog_id=5&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = 'd4303d22ceae4fbfe3d88c49ddba18693d7b19c1';
...
function mtSignIn() {
    ...
    var url = 'http://linker.in/mtos/mt-comments.cgi?__mode=login&blog_id=5';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Flinker.in%2F';
        }
    } else {
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://linker.in/mtos/mt-comments.cgi?__mode=handle_sign_in&static=0&logout=1';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
...
function mtShowGreeting() {
    ...
    if ( u && u.is_authenticated ) {
        if ( u.is_banned ) {
            ...
            var user_link;
            if ( u.is_author ) {
                user_link = '<a href="http://linker.in/mtos/mt-comments.cgi?__mode=edit_profile&return_url=' + encodeURIComponent( location.href );
                user_link += '">' + u.name + '</a>';
            } else {
